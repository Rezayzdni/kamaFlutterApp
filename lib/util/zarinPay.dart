  ...
  Future<Null> initUniLinks() async{
    ...
    try{
      _sub = getLinksStream().listen((event){
        ...
        showColoredToast(Colors.deepOrangeAccent, 'خطا در فرآیند پرداخت! لطفا دوباره امتحان کنید.');
        Navigator.pop(context);
...
android:host=x" />
  ...
  void configZarinPayment(){
    _paymentRequest
      ...
      ..setMerchantID("x")
      ..setAmount(35000)
      ..setCallbackURL("x") //The callback can be an android scheme or a website URL, you and can pass any data with The callback for both scheme and  URL
      ..setDescription("دسترسی به بخش پرمیوم برنامه بُمب کُدینگ");
  }
    ...
    ZarinPal().startPayment(_paymentRequest, (int status, String paymentGatewayUri) async{
      if(status == 100){
        ...
        if(isDoneUrl) {
          ...
          showColoredToast(Colors.deepOrangeAccent, 'خطا در ایجاد درخواست پرداخت!');
          Navigator.pop(context);
        }

      }
      else{
        showColoredToast(Colors.deepOrangeAccent, 'لطفا اتصال اینترنت رو چک کنید.');
        Navigator.pop(context);
      }
    });
  }
  void zarinPaymentVerification(String zarinPaymentStatus , String zarinPaymentAuthority){
    ZarinPal().verificationPayment(zarinPaymentStatus, zarinPaymentAuthority, _paymentRequest, (isPaymentSuccess,refID, paymentRequest){
      if(isPaymentSuccess){
        ...
        showColoredToast(Colors.deepOrangeAccent, 'فرآیند پرداخت با خطا مواجه شد.');
      }
